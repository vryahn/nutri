...
Tools: `search_catalog`, `log_entry` (by id or name; water = the "Agua" food, grams = ml), `delete_entry`, `get_day`, `get_targets` (with `bounds`), `set_target_bounds` (min/max per nutrient), `create_food` (per 100 g, hard-validates micro keys, soft warnings ⚠ same as the app), `create_recipe`, `update_food` (own food → update; base-catalog food → fork to own copy), `log_measurement`, `get_measurements`. Deleting foods and editing recipes are app-only by design.
